      ...
      <v-flex>
        <v-card class="mb-3">
          <v-toolbar flat dense>
            ...
            <v-btn outlined @click="newUserDialog()" class="mx-4"> <v-icon>note_add</v-icon> 수강생 추가 </v-btn>
            <v-btn outlined :loading="loader" @click="getListData()"> <v-icon>search</v-icon> 조회하기 </v-btn>
          </v-toolbar>
          <v-toolbar-items>
            <v-row dense class="pl-4 pr-4">
              <v-col sm="3" xs="12">
                <v-select dense v-model="search.role" :items="roleCodeList" item-text="dtlCdName" item-value="dtlCd" label="사용자 유형" @change="getTablesData" />
              </v-col>
              <v-col sm="3" xs="12">
          ...
          <v-data-table :headers="headers" :items="items" :page.sync="page" :items-per-page="itemsPerPage" @page-count="pageCount = $event" :items-per-page-options="[10, 20, 50, -1]">
            ...
            <template v-slot:item="{ item, index }">
              <tr @click="selectUserDetail(item)">
                ...
                <td>{{ $mycommon.convertPhoneString(item.user.phoneNo) }}</td>
                <td>{{ item.user.birthday }}</td>
                <td>
                  <v-chip small :color="userBadgeColor(item.user.role)">{{ detailCodeName[item.user.role] }}</v-chip>
                </td>
                <td>
                  <v-chip small :color="userBadgeColor(item.user.status)">{{ detailCodeName[item.user.status] }}</v-chip>
                </td>
            ...
            <v-card-text>
              <v-container grid-list-md>
                <v-layout wrap>
                  ...
                  <v-flex md3>
                    <v-select dense v-model="editItem.status" :rules="rules.requireRules" :items="acdmCodeList" item-text="dtlCdName" item-value="dtlCd" label="상태"></v-select>
                  </v-flex>
                  <v-flex md3>
                    <v-select dense v-model="editItem.agent" :items="roleCodeList" item-text="dtlCdName" item-value="dtlCd" label="소속"></v-select>
                  </v-flex>
                  <v-flex md3>
                    <v-select dense v-model="editItem.flag" :items="lessonCodeList" item-text="dtlCdName" item-value="dtlCd" label="부제"></v-select>
                  </v-flex>
                  <v-flex md3>
                    <v-select dense v-model="editItem.cardinalNo" :items="cardinalSelectList" item-text="dtlCdName" item-value="dtlCd" label="기수"></v-select>
                  </v-flex>
                  <v-flex md3>
                    <v-select dense v-model="editItem.carType" :items="carTypeSelectList" item-text="dtlCdName" item-value="dtlCd" label="차량유형"></v-select>
                  </v-flex>
                  <v-flex md3>
                    <v-select dense v-model="editItem.carModel" :items="carModelSelectList" item-text="dtlCdName" item-value="dtlCd" label="차종"></v-select>
                  </v-flex>
                  <v-flex md3>
                  ...
                  <v-flex md4>
                    <v-select dense v-model="editItem.engLevel" :items="courseCodeList" item-text="dtlCdName" item-value="dtlCd" label="영어등급"></v-select>
                  </v-flex>
                  <v-flex md4>
                    <v-select dense v-model="editItem.jpnLevel" :items="courseCodeList" item-text="dtlCdName" item-value="dtlCd" label="일어등급"></v-select>
                  </v-flex>
                  <v-flex md4>
                    <v-select dense v-model="editItem.chnLevel" :items="courseCodeList" item-text="dtlCdName" item-value="dtlCd" label="중국어등급"></v-select>
                  </v-flex>
                </v-layout>
              </v-container>
            </v-card-text>
...
export default {
  ...
  beforeMount() {
    ...
    //   this.search.role = 'STUDENT';
    // } else if (this.$route.path === '/teacher') {
    //   this.search.role = 'TEACHER';
    // }
    this.editItem = Object.assign({}, this.defaultItem);
    // this.itemsPerPage = this.$mycommon.getPagePerListCount();
  ...
  methods: {
    async getInitData() {
      ...
      let groupCode = await this.$mycommon.getCodeGroupList(['ROLE', 'USER', 'COURSE', 'ACDM', 'LESSON']); // <= 여러 개 불러올 때

      this.roleCodeList = await this.$mycommon.getGroupDetailList('ROLE'); // array 에 추가/삭제등이 있을 경우는 await this.$mycommon.getGroupDetailList 을 이용한다.
      this.userCodeList = await this.$mycommon.getGroupDetailList('USER'); // deep copy 가 필요하지 않다면 .slice() 로 새로운 배열을 만드는것으로 충분하다.
      this.courseCodeList = await this.$mycommon.getGroupDetailList('COURSE'); // groupCode 의 detailList 를 가져와 그 배열에 가공을 하게 되면 groupCode 의 detailList 도 같이 변경된다.
      this.acdmCodeList = await this.$mycommon.getGroupDetailList('ACDM'); // 의외로 javascript 가 call by reference 임을 생각한다.
      this.lessonCodeList = await this.$mycommon.getGroupDetailList('LESSON');

      this.roleCodeList.unshift(totalObj)
      // this.acdmCodeList.unshift(totalObj)
      // this.courseCodeList.unshift(totalObj)
      // this.userCodeList.unshift(totalObj)
      // this.lessonCodeList.unshift(selectObj)
    ...
    newDriverDialog() {
      ...
      this.courseCodeList.shift(totalObj);
      this.courseCodeList.unshift(selectObj);
      if (this.$refs.inputForm) {
        this.$refs.inputForm.resetValidation();
      }
    },
    ...
    dialogClose() {
      ...
      this.courseCodeList.shift(selectObj);
      this.courseCodeList.unshift(totalObj);
    },
    ...
    userBadgeColor(status) {
      if (status == 'USER_001') {
        return 'cyan darken-1';
      } else if (status == 'USER_002') {
        return 'red darken-1';
  ...
  watch: {
    // '$route.path' (to, from) { // 수강생 관리와 강사 관리에 동일한 컴포넌트를 쓰다보니 이게 필요하다.
