              ...
              <v-container>
                <v-row dense>
                  <v-col cols="12" sm="6" md="6">
                    <v-select dense v-model="editedItem.branchItem" :readonly="isBranchSelectReadonly" :items="branchList" label="지점 선택" item-text="name" item-value="branchId" return-object />
                  </v-col>
                  <v-col cols="12" sm="6" md="6">
                  ...
                  <v-col cols="12" sm="6" md="6">
                    <v-text-field v-model="editedItem.password" password hint="기본값: 폰번호 뒤 4자리" persistent-hint type="password" label="비밀번호" :rules="$rules.passWordRules" />
                  </v-col>
                  <v-col cols="12" sm="6" md="6">
                    <v-menu v-model="birthDatePicker" :close-on-content-click="false" :nudge-right="40" transition="scale-transition" offset-y>
                      <template v-slot:activator="{ on }">
                        <v-text-field v-model="editedItem.birthday" label="생년월일" v-mask="mask.dateMask" prepend-icon="event" @click:prepend="birthDatePicker = true" hint="날짜 선택 또는 직접입력(ex: 2002-01-01)" persistent-hint :rules="$rules.dateRules" />
                      </template>
                      <v-date-picker v-model="editedItem.birthday" @input="birthDatePicker = false" />
                    </v-menu>
                  </v-col>
                  <!-- <v-col cols="12" sm="6" md="6">
                    <v-text-field v-model="editedItem.status" label="상태" />
                  </v-col> -->
                </v-row>
              </v-container>
            </v-card-text>
